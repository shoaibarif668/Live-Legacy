...
const uploadHeader = () => {
    ...
    var formData = $("#header__upload__form").serializeArray(); //Form Data Distributing into array form 

    getFormValues(formData,jsonData) //Helper Function To get & set Values from Form
    console.log(jsonData)
    if(jsonData.galleryDescription && jsonData.galleryPage){
        $("#loader").show()
        axios
        .post(`${baseUrl}/api/user/manage-profile/update-gallery-information/${userData._id}`,jsonData,{headers: {'x-access-token': `${auth}`}})
        .then((response)=>{
